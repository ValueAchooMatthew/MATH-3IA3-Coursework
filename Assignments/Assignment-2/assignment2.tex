...
\mysection{3.2.17.) Show that $\lim_{n\to\infty} \frac{2^n}{n!} = 0$. Hint: If $n \ge 3$, then $0 < \frac{2^n}{n!} \le 2(\frac{2}{3})^{n-2}$.}
...
\section{Let $X = (x_n)$ be a sequence of positive real numbers such that $\lim_{n\to\infty}{\frac{x_{n+1}}{x_n}} = L > 1$. Show that X is not a bounded sequence and hence is not convergent.}
...
\begin{proof}
	...
	However, we had previously assumed that our sequence was bounded by M and thus $\forall n \in \mathbb{N} \; |x_n| < M $. Thus, by contradiction, our sequence X cannot be bounded and therefore, cannot be convergent.
\end{proof}
